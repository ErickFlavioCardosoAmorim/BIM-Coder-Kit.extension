...
def centro_geometria_elemento(elemento):
    """Retorna o centro espacial da caixa geometrica da luminaria IFC."""
    try:
...
def localizacao_elemento(elemento):
    """
    Retorna:
        ponto      = CENTRO GEOMETRICO/CAIXA da luminaria IFC
        angulo     = rotacao em torno do eixo Z, em radianos
        vetor      = direcao horizontal da luminaria

    IMPORTANTE:
        O ponto usado para insercao nao e mais prioritariamente o
        LocationPoint.Point. Ele passa a ser o centro da geometria
        (BoundingBox) da luminaria IFC.

        A rotacao continua sendo obtida do LocationPoint quando
        disponivel. Assim, posicao e orientacao ficam independentes.
    """

    # ---------------------------------------------------------
    # 1. PRIMEIRO: centro geometrico da luminaria IFC
    # ---------------------------------------------------------
    ponto_central = centro_geometria_elemento(elemento)

    # ---------------------------------------------------------
    # 2. ROTACAO: usa a rotacao real do LocationPoint, mas mantem
    #    o ponto central calculado acima.
    # ---------------------------------------------------------
    try:
        localizacao = elemento.Location
        ...
        if isinstance(localizacao, LocationPoint):
            ...
            if ponto_central:
                return ponto_central, angulo, vetor

    except Exception:
        pass

    # ---------------------------------------------------------
    # 3. Se nao houver LocationPoint, usa LocationCurve somente
    #    para descobrir a direcao.
    # ---------------------------------------------------------
    try:
        localizacao = elemento.Location
        ...
        if isinstance(localizacao, LocationCurve):
            ...
            if vetor.GetLength() > 0.000001:
                ...
                if vetor_xy.GetLength() > 0.000001:
                    ...
                    if ponto_central:
                        return ponto_central, angulo, vetor_xy

                    return curva.Evaluate(0.5, True), angulo, vetor_xy

    except Exception:
        pass

    # ---------------------------------------------------------
    # 4. Ultimo recurso: usa o vetor da geometria/BBox.
    # ---------------------------------------------------------
    vetor = vetor_direcao_elemento(elemento)

    if vetor and vetor.GetLength() > 0.000001:
        vetor = vetor.Normalize()
        vetor_xy = XYZ(vetor.X, vetor.Y, 0.0)
    ...
    if ponto_central:
        return ponto_central, 0.0, XYZ.BasisX
...
output.print_md("**Ponto de insercao:** centro geometrico (BoundingBox) de cada luminaria IFC.")
